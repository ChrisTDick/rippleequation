...
tf = pd.read_pickle("stocks3.pickle")
for colm in tf.columns: #drop columns with null values
    if tf[colm].isnull().values.any():
        tf = tf.drop(colm, axis = 1)
...
lf = pd.read_csv("lookup2.csv", index_col = 1, skiprows = 1, header = None, names = ["Ticker" , "one" , "two" , "three" , "four"])
names = dict(zip(lf.index, lf.Ticker)) #for full 505 only
lookup = {}
...
def extract_entities(text):
    try:
        orgs = [(X.text, X.label_) for X in text.ents if X.label_ in ["ORG", "GPE", "NORP"] ]
    except Exception as e:
        return "ERROR: " + str(e)
    return orgs

def sentiment_body(body):
    #initialise variables
    sentiment = Counter({})
    count     = {}
    if type(body) == float: return {}
    sentences = nlp(body).sents

    #iterate through each sentence in the body
    for sentence in sentences:
        e = clean_entities(extract_entities(sentence), with_type = False)
        e = [lookup[ent] for ent in e if ent in lookup.keys()]
        #if the sentence contains entities, calculate the sentiment
        if len(e) > 0:
            sentence_sentiment = Counter(calc_sentiment_returns([sentence.text] + e))
            #sentiment += sentence_sentiment

            #go through each entity in that sentence, add it to a master dict of sentiment
            for ent in sentence_sentiment.keys():
                if ent in sentiment.keys():
                    sentiment[ent] = max(sentiment[ent], sentence_sentiment[ent])
                    #count[ent]     += 1
                elif sentence_sentiment[ent] > 0.0:
                    sentiment[ent] = sentence_sentiment[ent]
                    #count[ent]     = 1

    #calc average sentiment for each entity
    #for ent in sentiment.keys():
    #    sentiment[ent] /= count[ent]
...
def config_timeseries(news_data, price_data, start_date, end_date, sentiment_func):
    ...
    #if finding sentiment in entire body...
    mf["sentiment"] = mf.nlp.apply(sentiment_body)
...
def predict(predict_date, num_ripples, thresh, adjacency_func, timeseries_data, moved_ticker = None,
    ...
    #initialise variables
    orig_graph      = build_graph(date = predict_date, thresh = thresh, timeseries_data = tf,
    ...
    R               = np.asarray(nx.adjacency_matrix(orig_graph).todense()) \
                      + np.diag(np.array([1]*len(orig_graph.nodes))) #influence matrix
    ...
    if verbose:
        print("Original:", nx.get_node_attributes(orig_graph,'px'))
        print("D:","\n" , D)
        print("R:", "\n", R, "\n")
    ...
    for ticker in moved_ticker:
        ...
        if verbose:
            print("moved ticker", ticker)
            print("moved_p_orig", moved_p_orig)
            print("moved_p_next", moved_p_next)
            print("moved_change", moved_change)
    ...
    for iteration in range(num_ripples):
        ...
        if verbose:
            ...
            print("p_t_minus_1:", p_t_minus_1)
            print("p_t_minus_2:", p_t_minus_2)
            print("D:","\n" , D)
            print("R:", "\n", R, "\n")

        #run one iteration of ripple equations
        p_t = np.diag(np.matmul(R,D)) * p_t_minus_1
        if verbose: print(p_t)
...
if __name__ == '__main__':
    ...
    start = datetime.now()
    print("Starting at:", start)
    timeseries_data = pd.read_pickle("timeseries_data.pickle")
    #timeseries_data = config_timeseries(news_data = df[cols], price_data = tf, start_date = start_date,
    ...
    print("\n", "****** BEGINNING TEST ******")
    middle = datetime.now()
    for thresh in [x/10.0 for x in range(2,10,1)]:
        for num_ripples in range(1,10):
            total_acc   = 0
            total_moved = 0

            for date in timeseries_data.index[1:]:
                acc_count   = 0
                moved_count = 0
            ...
            try:
                accuracy = total_acc / total_moved
            except:
                accuracy = 0.0

            result = (dampening, thresh, num_ripples, total_acc, total_moved, accuracy)
